---
description: "Iterative code review (multi-mode). codex/solo/dual selectable via --mode"
argument-hint: "[--mode <solo|codex|dual>] [--rounds N | --goal \"조건\"] <scope>"
---
...
- **solo**: Claude 서브에이전트 3개 병렬 리뷰 (SEC+ERR / DATA+PERF / CODE+IMPL), 폴백 시 순차 3-pass, 외부 AI 불필요
- **dual**: codex-cli 1차(SEC/ERR/DATA) + codex-cli 2차(PERF/CODE) 분할 독립 리뷰 — 두 호출 병렬 실행, Claude Code가 검증 및 수정

## 파라미터 (모드별)

| 파라미터 | codex (기본) | solo | dual |
|----------|-------------|------|--------|
| REVIEW_SKILL | `skills/code-review/SKILL.md` | `skills/code-review-solo/SKILL.md` | `skills/code-review/SKILL.md` |
| REVIEW_METHOD | codex 전 관점 독립 리뷰 | Claude 서브에이전트 3개 병렬 리뷰 (폴백: 순차 3-pass) | codex#1(SEC/ERR/DATA) ∥ codex#2(PERF/CODE) 병렬 |
| COMMIT_MSG_TAG | `(codex)` | `(solo)` | `(dual)` |

`--mode` 미지정 시 codex 모드를 사용합니다.

## 인수

- `$ARGUMENTS` 형식: `[--mode <solo|codex|dual>] [--rounds N | --goal "조건"] [scope]`
- `--mode <solo|codex|dual>` (선택): 리뷰 모드 선택 (기본: codex)
  - `codex`: codex-cli 전 관점 독립 리뷰 (기본값)
  - `solo`: Claude 서브에이전트 3개 다관점 병렬 리뷰 (폴백: 순차 3-pass, 외부 AI 불필요)
  - `dual`: codex-cli 1차(SEC/ERR/DATA) + codex-cli 2차(PERF/CODE) 분할 병렬 리뷰
...
2. value 검증: `solo`, `codex`, `dual` 중 하나
...
codex/dual 모드에서는 Claude가 파일 목록을 결정하지 않으며, 리뷰어가 자체적으로 codebase를 탐색합니다.
...
(dual 모드) 당신은 보안/에러 처리/데이터 일관성 전문 코드 리뷰어입니다. (PERF/CODE는 codex 2차 호출이 담당)
...
### codex-cli 2차 호출 (dual 모드 전용: PERF, CODE 관점)

> **codex/solo 모드에서는 이 섹션을 건너뜁니다.**

1차 호출과 **동시에 병렬로** 실행합니다 (별개 Bash 호출, 각각 `run_in_background: true`, BashOutput으로 완료 폴링). 두 호출은 서로 결과를 참조하지 않음 — 병렬 실행이므로 자연 충족됨. 각 호출의 출력은 BashOutput으로 개별 수집되므로 출력 파일 충돌 없음.
...
1차 호출 프롬프트를 **그대로 재사용**하되, 아래 3가지만 바꾼다 (나머지 — `## 리뷰 범위`, 라운드 1/2+ 범위 지정, git diff 파일 목록, `## 이전 라운드의 open finding` 래칫 블록 — 은 1차와 **바이트 동일**):

| # | 위치 | 1차 호출 | 2차 호출 (dual 전용) |
|---|------|---------|---------------------|
| 1 | `## 역할` 본문 첫 줄(들) | codex/dual 모드별 2줄 | `당신은 성능/코드 일관성 전문 코드 리뷰어입니다. (1차 codex 리뷰와 독립적으로 수행하며, 서로 결과를 참조하지 않습니다.)` 한 줄로 치환 |
| 2 | `## 전문 리뷰 관점` 자리 표시자 | `{리뷰 관점 블록}` | `{PERF/CODE 관점 블록}` |
| 3 | 프롬프트 말미 | 없음 | 아래 `## 역검토` 섹션을 추가 |
...
리뷰어(codex/solo/dual)가 발견한 각 finding을 직접 검증:

> **dual 모드 추가**: codex 1차와 codex 2차가 동일 이슈를 지적한 경우 — 같은 파일 + 라인 범위 겹침(±5줄) + 문제 유형 유사 → 같은 finding으로 통합. 더 높은 severity 채택, 양쪽 설명 병합.
...
2. **자체 탐색** (codex/dual 모드): 리뷰어에게 자연어 scope(R1) 또는 git diff 파일 목록(R2+)만 전달. Claude가 파일 목록을 결정하지 않음.
3. **독립 리뷰** (codex 모드): codex가 SEC/ERR/DATA/PERF/CODE 전 관점에서 독립 리뷰 수행
4. **관점 분리** (solo 모드): 각 서브에이전트(SEC/ERR, DATA/PERF, CODE/IMPL)는 담당 관점만 검토. 관점 혼합 금지. (순차 폴백 시에도 동일)
5. **분할 리뷰** (dual 모드): codex#1(SEC/ERR/DATA) ∥ codex#2(PERF/CODE) 병렬 호출. 서로 결과 참조 금지.

## 포기 방지 규칙 (강제)

- codex 호출 실패 시 → 재시도 1회, 이후에도 실패 시 Claude가 해당 관점 직접 리뷰
- codex 2차 호출 실패 시 (dual 모드) → 재시도 1회, 이후에도 실패 시 Claude가 PERF/CODE 직접 리뷰
