...
# 프로덕션 릴리즈 폴리싱 (2자 자동 토론형)

프로젝트를 프로덕션 릴리즈 준비 상태로 만듭니다.
codex-cli와 Claude Code가 **순차적으로 자동 토론**하여 모든 검증 통과 및 2자 합의할 때까지 반복합니다.
...
- `in_progress` -> `completed`: 2자 합의 또는 검증 통과 시
...
2. **Claude Code**가 codex 피드백 종합 후 **직접 수정**
...
**완료 조건**: 모든 검증 통과 + 2자 동시 합의 + `.claude-verification.json` 기록 완료
...
- **동일 피드백 3회 반복** -> codex 제외
- **근거 없는 approve 3회** -> codex 제외
