...
#### 6.5. ADR 기록 (아키텍처 결정 레코드 — 조건부)
**구조적 결정이 실제로 있을 때만** `docs/adr/NNN-<slug>.md`로 기록한다. 대상은 나중에 뒤집기 비싼 결정
— 프레임워크 선택, 저장소/영속화 전략, 인증·권한 방식, 서비스 간 통신 방식 등. ADR은 문서 충돌 시
**최우선 순위**(shared-rules.md 문서 우선순위 1위)로 참조되므로, 결정이 있었다면 여기에 남겨야 그 규칙이 작동한다.
형식 (파일당 ~10줄, 결정 1개당 1파일):
