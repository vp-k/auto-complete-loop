...
argument-hint: [--mode <solo|codex|gemini>] [definition(overview.md)] [doclist(README.md)]
---
...
| `gemini` | Claude + codex-cli + gemini 3자 리뷰 | codex-cli(리뷰) + gemini-cli(리뷰) + Claude Code(실행) |
...
- (옵션) `--mode <solo|codex|gemini>`: 리뷰 모드 선택 (기본값: codex)
- (옵션) 정의 문서 경로: $1
- (옵션) README 경로: $2
...
- `in_progress` -> `completed`: 모드별 합의(codex: 2자, solo: 검증자 확인, gemini: 3자) 또는 검증 통과 시
...
**gemini 모드**: codex-cli에게 검토 요청 → gemini에게 codex 피드백 검토 + 추가 피드백 요청 → Claude Code가 양측 종합 → 3자 합의까지 반복
...
**gemini 모드**: codex-cli에게 분석 요청 → gemini에게 추가 의견 요청 → Claude Code가 양측 종합 후 직접 수정
...
**gemini 모드**: codex-cli에게 보안 검토 요청 → gemini에게 추가 보안 검토 요청 → Claude Code가 종합 후 직접 수정
...
**gemini 모드**: codex-cli에게 검토 요청 → gemini에게 추가 검토 요청 → Claude Code가 종합 후 직접 정리
...
**완료 조건**: 모든 검증 통과 + 모드별 합의(codex: 2자, solo: 검증자 확인, gemini: 3자) + `.claude-verification.json` 기록 완료
...
## codex-cli 호출 방법 (codex/gemini 모드)
...
- **gemini 모드**: 참여 중인 AI 모두 근거 있는 "수정 없음" 선언, 또는 5회 이상 토론 후 주요 쟁점 해결
...
- 모든 외부 AI 제외 시 (codex/gemini 모드) → Claude Code가 단독으로 결정하고 마무리
