...
codex-cli, Gemini, Claude Code가 **순차적으로 자동 토론**하여 3자 모두 "수정 없음"에 합의할 때까지 반복합니다.
...
- `in_progress` -> `completed`: 3자 합의 완료 시
...
3. **gemini**에게 codex 피드백 검토 및 추가 피드백 요청
4. **Claude Code**가 양측 피드백 종합 분석/반론
   - 각 피드백의 타당성 검토
   - 수용할 피드백과 반론할 피드백 구분
   - 반론 시 근거와 대안 제시
5. codex-cli -> gemini 순서로 재검토/응답 (순차 호출)
6. 참여 중인 AI 모두 "수정 없음" 합의까지 2-5 반복
   - **각 라운드 완료 시** `.claude-plan-progress.json`의 `round` 값 업데이트
7. **문서 품질 체크리스트 확인** (합의 전 필수)
8. 합의된 내용으로 문서 수정/생성
9. 수정된 문서 다시 검토
   - 피드백 있으면 -> 2로 복귀
   - 피드백 없음 -> 문서 완성
...
## 요청
1. codex-cli 피드백 검토 (동의/반대)
2. 추가 피드백 제시
3. 우선순위별 분류 (Critical/High/Medium/Low)
```
...
- 제외된 AI는 해당 문서 토론에서 더 이상 호출하지 않음
- 남은 참여자들로 토론 계속 진행
...
- 토론이 교착 상태일 때 (모든 AI 제외됨, 7회 토론 초과)
- 3자 의견이 완전히 상충할 때
