...
# Full Auto Solo: 기획->구현->검수 올인원 (오케스트레이터)

한 줄 요구사항으로 **기획 문서 작성 -> 코드 구현 -> 코드 리뷰 -> 최종 검증**까지 자동 완주합니다.
외부 AI(codex, gemini) 없이 Claude Code가 다관점 검증으로 모든 단계를 수행합니다.

**역할 분담**: Claude = PM + 구현자 + 기획 토론(다관점) + 코드 리뷰(다관점)
...
이 파일 (오케스트레이터) — Ralph Loop, Phase 전이, Progress 관리 소유 (유일)
    ...
    ├── skills/doc-planning-solo/SKILL.md    (Phase 1 순수 로직 — solo 다관점)
    ├── skills/implementation/SKILL.md       (Phase 2 순수 로직)
    ├── skills/code-review-solo/SKILL.md     (Phase 3 순수 로직 — solo 다관점)
