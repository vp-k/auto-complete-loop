...
**검증**: `hooks/commit-msg.sh`가 US-suffix 형식을 검증. `[auto]` prefix 커밋에서 suffix 누락 시 에러 반환. 면제 키워드(`스캐폴딩|scaffolding|infrastructure|E2E 프레임워크|최종 검증|폴리싱|polishing`) 포함 시 통과.
