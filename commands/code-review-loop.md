...
- **dual**: codex-cli 1차(SEC/ERR/DATA) + codex-cli 2차(PERF/CODE) 분할 독립 리뷰, Claude Code가 검증 및 수정
...
| REVIEW_METHOD | codex 전 관점 독립 리뷰 | Claude 3-pass 순차 리뷰 | codex#1(SEC/ERR/DATA) + codex#2(PERF/CODE) |
...
- `--mode <solo|codex|dual>` (선택): 리뷰 모드 선택 (기본: codex)
  ...
  - `dual`: codex-cli 1차(SEC/ERR/DATA) + codex-cli 2차(PERF/CODE) 분할 리뷰
...
- **dual 모드**: 아래 "codex-cli 호출" 섹션(SEC/ERR/DATA만)과 "codex-cli 2차 호출" 섹션(PERF/CODE)을 순차 실행
...
5. **분할 리뷰** (dual 모드): codex#1(SEC/ERR/DATA) → codex#2(PERF/CODE) 순차 호출. 서로 결과 참조 금지.
