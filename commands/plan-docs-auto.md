...
- 정의 문서 경로: $1
- README 경로: $2
- `--mode <solo|codex|gemini>` (선택, 기본값: codex)
...
| `gemini` | Claude + codex-cli + gemini-cli 3자 토론 | codex-cli + gemini-cli가 피드백, Claude가 종합/수정 |

### --mode 처리

$ARGUMENTS에서 `--mode` 값을 파싱합니다. 지정되지 않으면 `codex`로 동작합니다.

- **`--mode solo`**: 3단계에서 외부 AI(codex-cli, gemini)를 호출하지 않고, Claude가 **작성자 ↔ 비판적 검토자 역할 전환**으로 자기 토론을 수행합니다. 수렴 기준: 3회 자기 토론 후 검토자가 Critical/High 0건 판단 시 완료. 최대 3회 반복.
- **`--mode codex`** (기본): 아래 3단계의 2자 토론 루프를 그대로 실행합니다.
- **`--mode gemini`**: 3단계에서 codex-cli와 gemini-cli를 모두 호출하여 3자 토론을 수행합니다. codex-cli -> gemini -> Claude Code 순서로 순차 검토/반론. 합의 기준: 참여 중인 AI 모두 근거 있는 "수정 없음" 선언.
