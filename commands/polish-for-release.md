---
description: "Pre-release polish (auto-discussion). codex-cli and Claude Code prepare release readiness"
argument-hint: [--mode <solo|codex|dual>] [definition(overview.md)] [doclist(README.md)]
---

# 프로덕션 릴리즈 폴리싱

프로젝트를 프로덕션 릴리즈 준비 상태로 만듭니다.
선택한 모드에 따라 리뷰 방식이 결정되며, 모든 검증 통과 및 합의할 때까지 반복합니다.
...
| 모드 | 설명 | 참여자 |
| ---- | ---- | ------ |
| `codex` (기본값) | Claude + codex-cli 2자 리뷰 | codex-cli(리뷰) + Claude Code(실행) |
| `solo` | Claude 실행자 + 검증자 역할 전환 | Claude Code 단독 (이중 역할) |
| `dual` | Claude + codex 1차 + codex 2차 3자 리뷰 | codex-cli 1차(리뷰) + codex-cli 2차(리뷰) + Claude Code(실행) |

## --mode 처리

$ARGUMENTS에서 `--mode` 플래그를 파싱합니다:
- `--mode solo` → solo 모드 활성화
- `--mode codex` 또는 `--mode` 미지정 → codex 모드 (기본값)
- `--mode dual` → dual 모드 활성화

나머지 인수(정의 문서 경로, README 경로)는 `--mode` 플래그를 제거한 후 순서대로 $1, $2에 할당합니다.
...
- codex는 절대 코드를 수정하지 않음. 피드백만 제공.

**solo 모드:**
- **Claude Code [실행자]**: 분석 + 코드 수정/작성 (실제 작업 수행)
- **Claude Code [검증자]**: 비판적 시각으로 수정 결과 검토 + 누락/부작용 확인
- 각 스텝에서 실행자가 작업 완료 후, 검증자 관점으로 전환하여 자체 검증.
- 외부 AI 불필요.

**dual 모드:**
- **codex-cli 1차 호출 역할**: 리뷰만 (문제 발견 및 피드백 제공)
- **codex 2차 호출 역할**: 리뷰만 (codex 1차 피드백 검토 + 추가 피드백 제공)
- **Claude Code 역할**: 분석 + 코드 수정/작성 (실제 작업 수행)
- codex 1·2차 호출 모두 절대 코드를 수정하지 않음. 피드백만 제공.

## 인수

- (옵션) `--mode <solo|codex|dual>`: 리뷰 모드 선택 (기본값: codex)
- (옵션) 정의 문서 경로: $1
- (옵션) README 경로: $2
...
- 나머지 인수를 순서대로 $1(정의 문서, 옵션), $2(README, 옵션)에 할당

### 복구 시 재개 규칙

1. `definitionDoc`, `readmePath` 확인 (인수와 일치해야 함)
2. `in_progress` 상태인 단계 찾기 -> 해당 단계부터 재개
3. `in_progress`가 없으면 첫 번째 `pending` 단계부터 재개
4. 모든 단계가 `completed`면 -> 완료 보고로 이동
...
- `in_progress` -> `completed`: 모드별 합의(codex: 2자, solo: 검증자 확인, dual: 3자) 또는 검증 통과 시
...
**codex 모드**: codex-cli에게 요구사항 충족 검토 요청 → Claude Code가 피드백 분석 → 2자 합의까지 반복
**solo 모드**: [실행자]가 요구사항 분석/수정 → [검증자]로 전환하여 검증 → 이슈 없을 때까지 반복
**dual 모드**: codex 1차 검토 요청 → codex 2차 추가 검토 요청(codex 1차 피드백 검토 + 추가 피드백) → Claude Code가 양측 종합 → 3자 합의까지 반복

### 누락/미충족 발견 시

- **Claude Code가 직접 수정** (codex는 수정하지 않음)
- 구현 완료까지 반복
- 구현 후 3~7단계 품질 검증 진행
...
**codex 모드**: codex-cli에게 문제 분석 요청 → Claude Code가 피드백 종합 후 직접 수정
**solo 모드**: [실행자]가 문제 분석/수정 → [검증자]로 전환하여 "새로운 문제가 생기지 않는가?" 체크
**dual 모드**: codex 1차 검토 요청 → codex 2차 추가 검토 요청 → Claude Code가 양측 종합 후 직접 수정
...
테스트 결과는 `quality-gate` / `e2e-gate` 서브커맨드가 `.claude-verification.json`에 자동 기록합니다 (verification.json 직접 편집은 가드가 차단함). 소프트 품질 차원을 추가로 기록할 때는:
```bash
bash ${CLAUDE_PLUGIN_ROOT}/scripts/shared-gate.sh record-dimension <key> <result> "<evidence>"
```
...
**codex 모드**: codex-cli에게 보안 검토 요청 → Claude Code가 종합 분석 후 직접 수정
**solo 모드**: [실행자]가 보안 취약점 분석/수정 → [검증자]로 전환하여 누락 확인
**dual 모드**: codex 1차 보안 검토 요청 → codex 2차 추가 보안 검토 요청 → Claude Code가 종합 후 직접 수정
...
**codex 모드**: codex-cli에게 정리 항목 검토 요청 → Claude Code가 종합 후 직접 정리
**solo 모드**: [실행자]가 정리 수행 → [검증자]로 전환하여 누락 확인
**dual 모드**: codex 1차 검토 요청 → codex 2차 추가 검토 요청 → Claude Code가 종합 후 직접 정리
...
**완료 조건**: 모든 검증 통과 + 모드별 합의(codex: 2자, solo: 검증자 확인, dual: 3자) + `.claude-verification.json` 기록 완료
...
## codex-cli 호출 방법 (codex/dual 모드)

> solo 모드에서는 외부 AI를 호출하지 않습니다.
...
### 요청
1. codex-cli 1차의 피드백을 검토하고 동의/반대 의견을 제시해주세요.
...
**AI 제외 규칙** (codex/dual 모드):

- **동일 피드백 3회 반복** -> 해당 AI 제외
- **근거 없는 approve 3회** -> 해당 AI 제외
- 모든 외부 AI 제외 시 Claude Code가 단독으로 결정하고 마무리
...
**합의 기준 (조기 종료 우선)** — 한 라운드에서 신규 Critical/High 0건이면 합의 성립. Medium/Low는 Claude가 타당성을 판단해 수용분만 반영하고 종료하며, "수정 없음" 선언을 추가 라운드로 확인받지 않는다 (최소 라운드 수 없음 — 1라운드 종료가 정상):

- **codex 모드**: codex 피드백에 신규 Critical/High 0건, 또는 5회 이상 토론 후 주요 쟁점 해결
- **solo 모드**: 검증자 판정에 신규 Critical/High 0건, 또는 5회 이상 검토 후 주요 쟁점 해결
- **dual 모드**: 양쪽 리뷰 모두 신규 Critical/High 0건, 또는 5회 이상 토론 후 주요 쟁점 해결
...
- 모든 외부 AI 제외 시 (codex/dual 모드) → Claude Code가 단독으로 결정하고 마무리
