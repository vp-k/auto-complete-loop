...
#### 6.5. ADR 기록 (아키텍처 결정 레코드)
**핵심 아키텍처 결정**(기술 스택, 저장소 전략, 인증 방식, 통신 방식 등 — 나중에 뒤집기 비싼 것)은
`docs/adr/NNN-<slug>.md`로 기록한다. ADR은 문서 충돌 시 **최우선 순위**(shared-rules.md 문서 우선순위 1위)로
참조되므로, 여기서 만들지 않으면 그 규칙이 공회전한다. 형식 (파일당 ~10줄, 결정 1개당 1파일):

```markdown
# ADR-001: <결정 제목>
- 상태: accepted
- 맥락: <왜 이 결정이 필요했나 — 1~2줄>
- 결정: <무엇을 선택했나>
- 대안과 기각 사유: <검토한 대안 → 왜 아닌가>
- 결과: <이 결정이 제약하는 것>
```

최소 1건(기술 스택 ADR)은 필수. Phase 1 토론에서 새 아키텍처 합의가 나오면 그때도 추가한다.
